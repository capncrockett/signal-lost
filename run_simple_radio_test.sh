...
# This script runs a simple radio test
...
if [ -z "$GODOT_EXECUTABLE" ]; then
    ...
    echo "Example: GODOT_EXECUTABLE=/path/to/godot ./run_simple_radio_test.sh"
    exit 1
fi
...
echo "Running Simple Radio Test..."
echo "Project path: $DIR"

# Run the test scene
cd "$DIR"
echo "Running test scene with verbose output..."
"$GODOT_EXECUTABLE" --path . --headless --verbose tests/SimpleRadioTestScene.tscn
