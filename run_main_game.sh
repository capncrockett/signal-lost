...
# This script runs the main game scene with the integrated radio interface
...
if [ -z "$GODOT_EXECUTABLE" ]; then
    ...
    echo "Example: GODOT_EXECUTABLE=/path/to/godot ./run_main_game.sh"
    exit 1
fi
...
echo "Running Main Game scene..."
echo "Project path: $DIR"

# Run the scene
cd "$DIR"
"$GODOT_EXECUTABLE" --path . --scene scenes/MainGameScene.tscn
