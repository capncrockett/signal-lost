...
# This script runs the Radio Audio Demo scene
...
if [ -z "$GODOT_EXECUTABLE" ]; then
    ...
    echo "Example: GODOT_EXECUTABLE=/path/to/godot ./run_radio_audio_demo.sh"
    exit 1
fi
...
echo "Running Radio Audio Demo scene..."
echo "Project path: $DIR"

# Run the scene
cd "$DIR"
"$GODOT_EXECUTABLE" --path . --scene scenes/RadioAudioDemo.tscn
