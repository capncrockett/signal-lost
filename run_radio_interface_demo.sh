...
# This script runs the Radio Interface Demo scene
...
if [ -z "$GODOT_EXECUTABLE" ]; then
    ...
    echo "Example: GODOT_EXECUTABLE=/path/to/godot ./run_radio_interface_demo.sh"
    exit 1
fi
...
echo "Running Radio Interface Demo scene..."
echo "Project path: $DIR"

# Run the scene
cd "$DIR"
"$GODOT_EXECUTABLE" --path . --scene scenes/RadioInterfaceDemo.tscn
