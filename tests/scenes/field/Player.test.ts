...
describe('Player', () => {
  ...
  describe('moveTo', () => {
    ...
    test('should play the correct animation', () => {
      ...
      // Move right
      player.moveTo(6, 5);
      expect(player.play).toHaveBeenCalledWith('player-walk-right');
      
      // Move left
      player.moveTo(4, 5);
      expect(player.play).toHaveBeenCalledWith('player-walk-left');
      
      // Move down
      player.moveTo(5, 6);
      expect(player.play).toHaveBeenCalledWith('player-walk-down');
      
      // Move up
      player.moveTo(5, 4);
      expect(player.play).toHaveBeenCalledWith('player-walk-up');
    });
  });
  ...
  describe('update', () => {
    ...
    test('should snap to target when close enough', () => {
      ...
      // @ts-ignore - Accessing private property for testing
      expect(player.x).toBe(player.targetX);
      // @ts-ignore - Accessing private property for testing
      expect(player.y).toBe(player.targetY);
