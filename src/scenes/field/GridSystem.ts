...
export class GridSystem {
  ...
  constructor(scene: Phaser.Scene, width: number, height: number, tileSize: number) {
    ...
    this.collisionGrid = Array(height).fill(null).map(() => Array(width).fill(false));
  }
  ...
  findPath(startX: number, startY: number, endX: number, endY: number): { x: number, y: number }[] | null {
    ...
    while (openSet.length > 0) {
      ...
      for (const neighbor of neighbors) {
        ...
        if (!this.isValidPosition(neighbor.x, neighbor.y) || this.isCollision(neighbor.x, neighbor.y)) {
          continue;
        }
