...
namespace SignalLost.UI
{
    ...
    public partial class PixelVisualizationManager : Node
    {
        ...
        public override void _Process(double delta)
        {
            ...
            if (EnableDynamicLighting && _lightingManager != null)
            {
                _lightingManager.EnableLighting = EnableDynamicLighting;
                
                // Sync ambient light with day/night cycle
                if (EnableDayNightCycle)
                {
                    _lightingManager.AmbientLightColor = GetAmbientLightColor();
                    _lightingManager.AmbientLightIntensity = GetAmbientLightIntensity();
                }
            }
        }
        ...
        public int CreateLight(Vector2 position, float radius, Color color, float intensity = 1.0f)
        {
            if (_lightingManager != null)
            {
                return _lightingManager.AddLight(position, radius, color, intensity);
            }
            return -1;
        }

        // Create a temporary light at a specific position
        public int CreateTemporaryLight(Vector2 position, float radius, Color color, float intensity = 1.0f, float duration = 2.0f)
        {
            if (_lightingManager != null)
            {
                return _lightingManager.AddTemporaryLight(position, radius, color, intensity, duration);
            }
            return -1;
        }

        // Create a flickering light at a specific position
        public int CreateFlickeringLight(Vector2 position, float radius, Color color, float intensity = 1.0f, float flickerInterval = 0.1f)
        {
            if (_lightingManager != null)
            {
                return _lightingManager.AddFlickeringLight(position, radius, color, intensity, flickerInterval);
            }
            return -1;
        }

        // Create a pulsing light at a specific position
        public int CreatePulsingLight(Vector2 position, float radius, Color color, float intensity = 1.0f, float pulseSpeed = 1.0f)
        {
            if (_lightingManager != null)
            {
                return _lightingManager.AddPulsingLight(position, radius, color, intensity, pulseSpeed);
            }
            return -1;
        }
        ...
        public void RemoveLight(int id)
        {
            if (_lightingManager != null)
            {
                _lightingManager.RemoveLight(id);
            }
        }

        // Get lighting at a specific position
        public (Color color, float intensity) GetLightingAt(Vector2 position)
        {
            if (_lightingManager != null)
            {
                return _lightingManager.CalculateLightingAt(position);
            }
            return (GetAmbientLightColor(), GetAmbientLightIntensity());
        }
