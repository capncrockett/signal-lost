...
# This script runs the Pixel Map Test scene
...
if [ -z "$GODOT_EXECUTABLE" ]; then
    ...
    echo "Example: GODOT_EXECUTABLE=/path/to/godot ./run_pixel_map_test.sh"
    exit 1
fi
...
echo "Running Pixel Map Test scene..."
echo "Project path: $DIR"

# Run the scene
cd "$DIR"
"$GODOT_EXECUTABLE" --path . --scene scenes/test/PixelMapTest.tscn
