...
namespace SignalLost.Utils
{
    ...
    public partial class ScreenshotAnalyzer : Node
    {
        ...
        private (Color AverageColor, float Brightness, bool IsDark, bool HasRed, bool HasGreen, bool HasBlue) AnalyzeColors(Image image)
        {
            ...
            var resizedImage = Image.CreateEmpty(100, 100, false, Image.Format.Rgba8);

            // Copy the image data manually
            for (int y = 0; y < Math.Min(resizedImage.GetHeight(), image.GetHeight()); y++)
            {
                for (int x = 0; x < Math.Min(resizedImage.GetWidth(), image.GetWidth()); x++)
                {
                    resizedImage.SetPixel(x, y, image.GetPixel(x, y));
                }
            }

            resizedImage.Resize(100, 100, Image.Interpolation.Bilinear);
