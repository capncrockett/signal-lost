...
namespace SignalLost
{
    ...
    public partial class PixelRadioInterface : Control
    {
        ...
        // References to parent RadioTuner
        ...
        public override void _Ready()
        {
            ...
            if (_radioTuner != null)
            {
                _minFrequency = _radioTuner.MinFrequency;
                _maxFrequency = _radioTuner.MaxFrequency;
        ...
        {
            // Update knob rotation based on frequency
            if (_radioTuner != null)
            {
        ...
        private void UpdateFrequencyFromSliderPosition(float mouseX)
        {
            float sliderX = _frequencySliderRect.Position.X;
            float sliderWidth = _frequencySliderRect.Size.X;
            
            // Calculate percentage along slider
            float percentage = Mathf.Clamp((mouseX - sliderX) / sliderWidth, 0, 1);
            ...
            newFrequency = Mathf.Snapped(newFrequency, 0.1f);
            
            // Set new frequency
            _radioTuner?.SetFrequency(newFrequency);
        }
